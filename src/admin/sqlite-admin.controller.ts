 ...
 *
 * All query endpoints block destructive SQL (DELETE, UPDATE, DROP, etc.)
...
#sidebar h2{font-size:14px;color:#58a6ff;margin-bottom:8px;padding-bottom:8px;border-bottom:1px solid #30363d}
...
<div id="app">
  <div id="sidebar">
    <h2>SQLite Admin</h2>
    <div id="meta" class="l">connecting...</div>
    <h3>Tables</h3>
    <div id="tables"></div>
  </div>
  <div id="main">
...
async function info(){
  ...
  catch(e){document.getElementById('meta').innerHTML='<span style="color:#f85149">err</span>';}}

async function list(){
  try{const d=await(await fetch(API+'/tables')).json();tables=d.tables;
    ...
    tables.forEach(function(tbl){const o=document.createElement('option');o.value=tbl.name;o.textContent=tbl.name+(tbl.c?' ('+tbl.c+')':'');s.appendChild(o);
      ...
      t.appendChild(d2);});}
  catch(e){document.getElementById('tables').innerHTML='<span style="color:#f85149">err</span>';}
}
...
async function runQuery(){
  ...
  const t0=Date.now();
  try{const r=await fetch(API+'/query',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({sql})});
...
export class SQLiteAdminController {
    ...
    /** Serve the embedded SQLite admin UI (single HTML page) */
    @Get()
    ...
    /** List tables with row counts (excludes SQLite internals) */
    @Get('tables')
    ...
    /** Execute read-only SQL, return JSON results */
    @Post('query')
